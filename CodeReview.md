...
React 19 + TypeScript, Vite, Tailwind CSS 4, React Router, Recharts, React-Leaflet, `@turf` for geospatial analysis, and `@react-pdf/renderer` for PDF generation.
...
- **Sankey diagram**: `ReceiptSankey.tsx` implements a full 3-column Recharts `Sankey` with custom node rendering (name + dollar label, intelligent edge-aware label positioning) and hover interactivity — hovering a node or link dims unrelated nodes/links via `connectedIndices` / `activeLinkIndices`. `SankeyBlock.tsx` drives data transformation, shows a "fill in the form first" placeholder when no data exists, and forwards `containerRef` to the SVG for PDF capture
